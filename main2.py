...
# ---------------------------------------------------------------------
# 2. Initialize ICD→CCS mapper (Transform: code normalization)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 3. Patient filtering (length of stay, non-overlap) – matches paper
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 4. Map visits to multi-level CCS codes (Transform)
# ---------------------------------------------------------------------

# None = multi-level CCS (1, 1.1, 1.1.1, 1.1.2.1, ...)
# 1 / 2 / 3 / 4 = specific CCS level only
CCS_LEVEL = None  # USE MULTI-LEVEL CCS (like in ICE-NODE paper)


def map_visit_to_ccs_codes(visit):
    """Map visit ICD codes to CCS codes using our custom mapper"""
    codes = set()
    icd_codes = []

    # Pull ICD diagnosis codes from PyHealth visit
...
def build_patient_visit_ccs_index(dataset, filtered_patient_ids):
    """Build map: (patient_id, visit_id) → list of CCS codes."""
    visit_ccs = {}
...
# ---------------------------------------------------------------------
# 5. Build CCS vocabulary (multi-level)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 6. Build patient time series (Transform)
# ---------------------------------------------------------------------
def build_patient_timeseries(dataset, filtered_patient_ids, visit_ccs, ccs_code_to_index):
    """
    For each patient:
      - sort visits by discharge_time
      - compute days since first discharge
      - build multi-hot CCS vector for each visit
    """
    patient_timeseries = {}
    for patient_id in filtered_patient_ids:
        ...
        for visit_id, visit in patient.visits.items():
            ...
            if discharge_time is None:
                continue
            timed_visits.append((discharge_time, visit_id, visit))

        if not timed_visits:
            continue

        timed_visits.sort(key=lambda x: x[0])
        first_time = timed_visits[0][0]

        visit_sequence = []
        for event_time, visit_id, visit in timed_visits:
            days_since_first = (event_time - first_time).total_seconds() / 86400.0
            ...
            visit_sequence.append((days_since_first, x))

        if len(visit_sequence) >= 2:
            patient_timeseries[patient_id] = visit_sequence

    return patient_timeseries


mimic3_timeseries = build_patient_timeseries(
    mimic3, mimic3_filtered_ids, mimic3_visit_ccs, ccs_code_to_index
)
mimic4_timeseries = build_patient_timeseries(
    mimic4, mimic4_filtered_ids, mimic4_visit_ccs, ccs_code_to_index
)
...
# ---------------------------------------------------------------------
# 7. Save processed data (Load into your “processed” layer)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 8. Train/val/test patient splits (like paper)
# ---------------------------------------------------------------------
def split_patients(patient_ids, seed=0, train_ratio=0.7, val_ratio=0.15):
    random.seed(seed)
...
m3_train_ids, m3_val_ids, m3_test_ids = split_patients(m3_ids, seed=0)
m4_train_ids, m4_val_ids, m4_test_ids = split_patients(m4_ids, seed=0)

print(
    f"\nMIMIC-III train/val/test split: "
    f"{len(m3_train_ids)}/{len(m3_val_ids)}/{len(m3_test_ids)}"
)
print(
    f"MIMIC-IV train/val/test split: "
    f"{len(m4_train_ids)}/{len(m4_val_ids)}/{len(m4_test_ids)}"
)
