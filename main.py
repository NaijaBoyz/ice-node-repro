...
print("MIMIC-III filtered patient count:", len(mimic3_filtered_ids))
...
def map_visit_to_ccs_codes(visit):
    ...
    for code in icd_codes:
        code_str = str(code)
        try:
            mapped = icd9_to_ccs.map(code_str, target_kwargs={"level": CCS_LEVEL})
            if mapped:
                codes.update(mapped)
                continue
        except:
            pass
        try:
            mapped = icd10_to_ccs.map(code_str, target_kwargs={"level": CCS_LEVEL})
            if mapped:
                codes.update(mapped)
        except:
            pass
...
def build_patient_visit_ccs_index(dataset, filtered_patient_ids):
    ...
    for pid in filtered_patient_ids:
        ...
        for vid, visit in patient.visits.items():
            visit_ccs[(pid, vid)] = map_visit_to_ccs_codes(visit)
    return visit_ccs
mimic3_visit_ccs = build_patient_visit_ccs_index(mimic3, mimic3_filtered_ids)
print("MIMIC-III visits mapped to CCS codes:", len(mimic3_visit_ccs))
mimic4_visit_ccs = build_patient_visit_ccs_index(mimic4, mimic4_filtered_ids)
print("MIMIC-IV visits mapped to CCS codes:", len(mimic4_visit_ccs))
...
print("CCS categories in vocabulary:", len(sorted_ccs_codes))
...
print("MIMIC-III patients with valid timeseries:", len(mimic3_timeseries))
print("MIMIC-IV patients with valid timeseries:", len(mimic4_timeseries))
PROCESSED_DIR = Path("data/processed")
PROCESSED_DIR.mkdir(exist_ok=True, parents=True)
with open(PROCESSED_DIR / "ccs_vocab.json", "w") as f:
    json.dump(sorted_ccs_codes, f)
with open(PROCESSED_DIR / "mimic3_timeseries.pkl", "wb") as f:
    pickle.dump(mimic3_timeseries, f)
with open(PROCESSED_DIR / "mimic4_timeseries.pkl", "wb") as f:
...
print("MIMIC-III train/val/test split:", len(m3_train_ids), len(m3_val_ids), len(m3_test_ids))
print("MIMIC-IV train/val/test split:", len(m4_train_ids), len(m4_val_ids), len(m4_test_ids))
m3_splits = {"train": m3_train_ids, "val": m3_val_ids, "test": m3_test_ids}
with open(PROCESSED_DIR / "mimic3_splits.pkl", "wb") as f:
    pickle.dump(m3_splits, f)
m4_splits = {"train": m4_train_ids, "val": m4_val_ids, "test": m4_test_ids}
with open(PROCESSED_DIR / "mimic4_splits.pkl", "wb") as f:
