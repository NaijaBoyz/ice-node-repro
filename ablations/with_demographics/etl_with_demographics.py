...
def extract_patient_demographics(patient):
    demographics = {}
    
    gender = getattr(patient, "gender", None)
    if gender is None or gender == "":
        demographics["gender"] = 0.5
    elif str(gender).upper() in ["M", "MALE"]:
        demographics["gender"] = 1.0
    elif str(gender).upper() in ["F", "FEMALE"]:
        demographics["gender"] = 0.0
    else:
        demographics["gender"] = 0.5
    
    birth_time = getattr(patient, "birth_datetime", None)
    if birth_time is not None:
        demographics["birth_year"] = birth_time.year
    else:
        demographics["birth_year"] = None
...
def build_patient_timeseries_with_demographics(dataset, filtered_patient_ids, visit_ccs, ccs_code_to_index):
    patient_timeseries = {}
    ...
    for patient_id in filtered_patient_ids:
        ...
        last_admission_time = timed_visits[-1][0]
        
        total_duration_days = (last_admission_time - first_admission_time).total_seconds() / 86400.0
        ...
        if len(visit_sequence) >= 2:
            if demographics["birth_year"] is not None:
                age_at_first_admission = first_admission_time.year - demographics["birth_year"]
                normalized_age = min(age_at_first_admission / 100.0, 1.0)
            else:
                normalized_age = 0.5
            
            relative_duration = min(total_duration_days / 365.0, 1.0)
...
print("\n" + "="*80)
print("BUILDING TIMESERIES WITH DEMOGRAPHIC FEATURES")
print("="*80)

mimic3_timeseries = build_patient_timeseries_with_demographics(
    mimic3, mimic3_filtered_ids, mimic3_visit_ccs, ccs_code_to_index
)
mimic4_timeseries = build_patient_timeseries_with_demographics(
...
print("\nDemographic Statistics (MIMIC-III):")
genders = [p["demographics"]["gender"] for p in mimic3_timeseries.values()]
ages = [p["demographics"]["normalized_age"] for p in mimic3_timeseries.values()]
print(f"  Gender distribution: {sum(1 for g in genders if g == 1.0)} male, {sum(1 for g in genders if g == 0.0)} female, {sum(1 for g in genders if g == 0.5)} unknown")
print(f"  Average normalized age: {sum(ages) / len(ages):.3f}")
...
print(f"\nProcessed data with demographics saved to {PROCESSED_DIR}")
...
print("\n" + "="*80)
print("DATA PREPARATION WITH DEMOGRAPHICS COMPLETE!")
print("="*80)
print("\nDemographic features included:")
print("  - gender: 0.0 (female), 1.0 (male), 0.5 (unknown)")
print("  - normalized_age: age at first admission / 100")
print("  - relative_duration: total trajectory duration / 365 days")
